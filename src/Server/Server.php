...
if (!defined('WP_GRAPHQL_GUTENBERG_SERVER_USER_ROLE_DISPLAY_NAME')) {
    define('WP_GRAPHQL_GUTENBERG_SERVER_USER_ROLE_DISPLAY_NAME', __('WP GraphQL Gutenberg Server User', 'wp-graphql-gutenberg'));
}
...
class Server
{
    ...
    protected function fetch($path, $data = [])
    {
        ...
        $expiration = time() + 60;
        $cookie = wp_generate_auth_cookie(self::ensure_server_user()->ID, $expiration, $secure ? 'secure_auth' : 'auth');
        ...
        if (is_wp_error($result)) {
            throw new ServerException($result->get_error_messages());
        }
