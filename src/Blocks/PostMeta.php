...
class PostMeta {
	public static function is_data_stale( $id, $data ) {
		if ( get_post( $id )->post_content !== $data['post_content'] ) {
			return true;
		}
	...
	public static function update_post( $id, $post_content, $blocks, $registry ) {
		$meta = wp_slash( json_encode( self::prepare_meta( $id, $post_content, $blocks, $registry ) ) );

		update_metadata( 'post', $id, WP_GRAPHQL_GUTENBERG_DATA_META_FIELD_NAME, $meta );
	}

	public static function get_post( $id ) {
		$meta = get_metadata( 'post', $id, WP_GRAPHQL_GUTENBERG_DATA_META_FIELD_NAME, true );
